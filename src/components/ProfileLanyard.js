...
const SPRING_CONFIG = { stiffness: 120, damping: 12, mass: 1 };

const ProfileLanyard = () => {
  // Motion Values untuk physics lanjut
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);

  // Inisialisasi Spring untuk pengembalian yang halus
  const xSpring = useSpring(mouseX, SPRING_CONFIG);
  const ySpring = useSpring(mouseY, SPRING_CONFIG);

  // Rotasi Dinamis berdasarkan posisi drag (Efek Tilt Realistik)
  const rotateX = useTransform(ySpring, [-120, 120], [10, -10]);
  const rotateY = useTransform(xSpring, [-120, 120], [-10, 10]);

  // Efek Peregangan Tali (Stretch Lanyard)
  ...
  return (
    <div className="flex flex-col items-center justify-start relative pt-32 pb-20 min-h-[600px] w-full" style={{ perspective: 1000 }}>
      {/* Tali Lanyard Realtime (SVG untuk tampilan realistik) */}
      <svg
      ...
      {/* Komponen Kartu yang Dapat Di-drag */}
      <motion.div
        drag
      ...
      >
        ...
        <div className="glass-card w-[280px] p-6 rounded-2xl border border-white/10 backdrop-blur-md bg-white/5 shadow-[0_0_25px_rgba(34,211,238,0.15)] relative overflow-hidden">
          {/* Lubang atas untuk koneksi tali */}
          <div className="absolute top-3 left-1/2 -translate-x-1/2 w-8 h-2 bg-white/10 rounded-full border border-white/5" />

          <div className="flex flex-col items-center mt-4">
            {/* Pembungkus Foto Profil */}
            ...
            <div className="text-center">
              ...
              <div className="mt-8 pt-4 border-t border-white/10 w-full flex flex-col items-center opacity-30">
                <div className="h-8 w-full bg-[repeating-linear-gradient(90deg,white,white_1px,transparent_1px,transparent_4px)]" />
          ...
          {/* Efek Shine Gilap */}
          <div className="absolute -inset-full bg-gradient-to-tr from-transparent via-white/5 to-transparent rotate-45 pointer-events-none transition-transform duration-1000 group-hover:translate-x-full" />
        </div>
      </motion.div>
    </div>
  );
