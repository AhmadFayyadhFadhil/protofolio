...
const Intro = ({ onFinish }) => {
  ...
  useEffect(() => {
    ...
    // Bungkus setiap karakter dalam span untuk animasi stagger GSAP
    if (textRef.current) {
    ...
    // 1. Inisialisasi: Efek breathing glow & ripple noise
    gsap.to(".glow-orb", {
      scale: 1.2,
      opacity: 0.8,
      duration: 3,
    ...
    // Cleanup GSAP animations and timers on unmount
    return () => {
      tl.kill();
      gsap.killTweensOf(".glow-orb");
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 z-[100] flex items-center justify-center bg-black overflow-hidden select-none">
      ...
      {/* Slide Overlay (Transisi Penutup) */}
      <div ref={overlayRef} className="absolute inset-0 bg-darkBg z-20 translate-y-full" />
    </div>
  );
};
